...
public:
     ...
     */
    UniformSample(std::uint32_t reservoirSize = DEFAULT_SAMPLE_SIZE);
    ...
    static const std::uint64_t DEFAULT_SAMPLE_SIZE;
private:
    std::uint64_t getRandom(std::uint64_t count) const;
    const std::uint64_t reservoir_size_;
    std::atomic<std::uint64_t> count_;
    typedef std::vector<std::int64_t> Int64Vector;
    Int64Vector values_;
