...
std::uint64_t Meter::Impl::getCount() const {
    return count_;
}
...
void Meter::Impl::tickIfNecessary() {
    std::uint64_t old_tick = last_tick_;
    std::uint64_t cur_tick =
            std::chrono::duration_cast<std::chrono::nanoseconds>(
                    Clock::now().time_since_epoch()).count();
    std::uint64_t age = cur_tick - old_tick;
    if (age > TICK_INTERVAL) {
        std::uint64_t new_tick = cur_tick - age % TICK_INTERVAL;
        if (last_tick_.compare_exchange_strong(old_tick, new_tick)) {
            std::uint64_t required_ticks = age / TICK_INTERVAL;
            for (std::uint64_t i = 0; i < required_ticks; i++) {
                tick();
            }
        }
    }
...
std::uint64_t Meter::getCount() const {
